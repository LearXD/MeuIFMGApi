...
import { extractSubjects, extractWarnings } from '../utils/HtmlParser.js';
...
api.get('/', middleware, async (req, res, next) => {
    ...
    try {
        ...
        const { SERVER_HOST, SERVER_WARNINGS_ROUTE } = process.env
        ...
        const data = extractWarnings(response.data, token);
        if (!data) {
